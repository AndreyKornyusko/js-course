...
class Hamburger {
  ...
  removeTopping(topping) {
    let toppingIndex = this._toppings.indexOf(topping);
    if (toppingIndex != -1) {
  ...
  get price() {
    return (
      this._toppings
        .map(value => Hamburger.TOPPINGS[value].price)
  ...
  get calories() {
    return (
      this._toppings
        .map(value => Hamburger.TOPPINGS[value].calories)
        .reduce((acc, value) => acc + value, 0) +
...
Hamburger.STUFFING_MEAT = 'TUFFING_MEAT';
...
// Cмотрим начинку
console.log('Начинка', hamburger.stuffing);
