...
const getUsersByEyeColor = (arr, color) => {
  return arr.filter(arr => arr.eyeColor === color);
};
...
const getUsersByGender = (arr, gender) => {
  return arr.filter(arr => arr.gender === gender);
};
...
const getInactiveUsers = arr => {
  return arr.filter(arr => arr.isActive === false);
};
...
const getUserByEmail = (arr, email) => {
  return arr.find(arr => arr.email === email);
};
...
const getUsersWithAge = (arr, min, max) => {
  return arr.filter(arr => arr.age >= min && arr.age <= max);
};
...
const getUsersByFriend = (arr, name) => {
  return arr.reduce(
    (acc, value) =>
      value.friends.includes(name) ? acc.concat(value.name) : acc,
    [],
  );
};
...
const getAllSkills = arr => {
  return arr
    .reduce(
      (acc, value) =>
        !acc.includes(value.skills) ? acc.concat(value.skills) : acc,
      [],
    )
    .reduce(
      (acc, value) => (!acc.includes(value) ? acc.concat(value) : acc),
      [],
    )
    .sort();
};
...
const getUserNamesSortedByFriendsCount = arr => {
  return arr
    ...
    .reduce((acc, value) => acc.concat(value.name), []);
};
